...
eststo clear
eststo: quietly xtreg Flexsum tp, cluster(ind)
eststo: quietly reghdfe Flexsum tp, absorb(Stkcd date city#date) cluster(ind)
eststo: quietly xtreg Flexsum tp Boardsize size indp dual salegr roa tobinq c.degree##i.date, cluster(ind)
eststo: quietly reghdfe Flexsum tp Boardsize size indp dual salegr roa tobinq c.degree##i.date, absorb(Stkcd date city#date) cluster(ind)
esttab, star(* 0.1 ** 0.05 *** 0.01) ///
    b(%8.4f) se(%6.4f) ///
    title("Regression Results") ///
    label stats(N r2_a, fmt(0 4) labels("N" "Adj-R2"))
esttab using "更换固定效应聚类回归.csv", replace nogaps ///
    b(%8.4f) se(%6.4f) ///
    star(* 0.1 ** 0.05 *** 0.01) ///
    level(95) ///
...
reghdfe Flexsum pre_* current post_* , absorb(Stkcd date city#date) cluster(ind)
coefplot, baselevels ///
